...
class Strobe:
    def __init__(self, handler):
        ...
        self.delay_adjustment = Gtk.Adjustment(0.00, 0.0, 1.0, 0.01, 0.1, 0)
        ...
        self.duty_adjustment = Gtk.Adjustment(0.00, 0.0, 1.0, 0.01, 0.1, 0)
        ...
        self.grid.attach(self.smoothness_scale, 0, 1, 1, 1)
        self.grid.attach(self.delay_scale, 0, 2, 1, 1)
        self.grid.attach(self.duty_scale, 0, 3, 1, 1)

        self.strobes_box.pack_start(self.grid, False, False, 0)

        # Group select
        group_grid = Gtk.Grid()
        for i in range(len(handler.controller.groups)):
            checkbox = Gtk.CheckButton(str(i + 1))
            checkbox.connect("toggled", self.group_selected_cb, i)
            group_grid.attach(checkbox, i % 5, i // 5, 1, 1)

        self.grid.attach(group_grid, 0, 4, 1, 1)

        self.switch = Gtk.Switch()
        self.switch.connect("state-set", self.switch_clicked_cb)
        self.grid.attach(self.switch, 0, 5, 1, 1)

        remove_button = Gtk.Button("Remove")
        remove_button.connect("clicked", self.remove_button_cb)
        self.grid.attach(remove_button, 1, 6, 1, 1)

        self.grid.attach(Gtk.Separator(), 0, 7, 1, 1)
    ...
    def color_chosen(self, button, name):
        ...
        def rainbow_provider():
            for i in self.handler.controller.color.rainbow_colors():
                yield i
    ...
    def start(self):
        self.run = True
        GObject.timeout_add(1000 * self.delay, self.send_strobe, None)
        pass
    ...
    def send_strobe(self, _):
        ...
        self.handler.controller.network.send(state)

        def restore(_):
            self.handler.controller.network.send(self.handler.controller.network.generator())
            return False
